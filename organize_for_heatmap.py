...
#bones for the output dataframe, where sentiment's aggregated by date for each state
big_time_state_df = pd.DataFrame( 
	columns = ['date', 'state', 'frequency', 'sentiment'])

#make sure we're using full tweets (the deleted ones from every source but historical
#will be truncated--see README for more info)
